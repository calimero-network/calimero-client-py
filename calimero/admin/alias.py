...
class AliasManager:
    ...
    async def create_context_alias(
        self, name: str, context_id: str
    ) -> CreateAliasResponse:
        """
        ...
        """
        payload = {"name": name, "contextId": context_id, "type": "context"}
    ...
    async def create_application_alias(
        self, name: str, application_id: str
    ) -> CreateAliasResponse:
        """
        ...
        Returns:
            The create alias response.
        """
        payload = {"name": name, "applicationId": application_id, "type": "application"}
        result = await self.client._make_request("POST", "/admin-api/aliases", payload)
        if isinstance(result, dict) and (result.get("success") or "data" in result):
            # Add success field if it doesn't exist, so the workflow engine can access it
            if "success" not in result:
    ...
    async def create_identity_alias(
        self, context_id: str, name: str, identity_id: str
    ) -> CreateAliasResponse:
        """
    ...
    async def lookup_identity_alias(
    ...
    async def list_context_aliases(self) -> ListAliasesResponse:
        ...
        if isinstance(result, dict) and (result.get("success") or "data" in result):
            ...
            return ListAliasesResponse(
                success=True, aliases=aliases, total_count=len(aliases)
            )
        else:
    ...
    async def list_application_aliases(self) -> ListAliasesResponse:
        ...
        result = await self.client._make_request(
            "GET", "/admin-api/aliases/applications"
        )
        if isinstance(result, dict) and (result.get("success") or "data" in result):
            aliases_data = result.get("data", [])
            aliases = [
    ...
    async def list_identity_aliases(self, context_id: str) -> ListAliasesResponse:
        ...
        """
        result = await self.client._make_request(
            "GET", f"/admin-api/aliases/contexts/{context_id}/identities"
        )
        if isinstance(result, dict) and (result.get("success") or "data" in result):
            aliases_data = result.get("data", [])
            aliases = [
    ...
    async def delete_context_alias(self, name: str) -> DeleteAliasResponse:
        ...
        if isinstance(result, dict) and (result.get("success") or "data" in result):
            return DeleteAliasResponse(success=True, alias_name=name)
        else:
