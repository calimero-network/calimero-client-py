...
class SystemManager:
        ...
        if isinstance(result, dict):
            ...
            if result.get("success") or "data" in result:
                ...
                return IsAuthenticatedResponse(
                    success=True, authenticated=data.get("authenticated", False)
                )
            ...
            if result.get("success") or "data" in result or "count" in result:
                ...
                if "count" in result and "data" not in result:
                    return GetPeersResponse(
                        success=True, peers=[], total_count=result.get("count", 0)
                    )
    ...
    async def get_peers_count(self) -> GetPeersCountResponse:
        ...
        if isinstance(result, dict):
            ...
            if result.get("success") or "data" in result or "count" in result:
                ...
                return GetPeersCountResponse(success=True, peers_count=count)
            elif (
                result.get("success") == False or result.get("success") == "False"
                ...
                if "404" in str(result.get("error", "")):
                    ...
                    try:
                        ...
                        return GetPeersCountResponse(
                            success=True, peers_count=peers_result.total_count
                        )
                    except:
                        # If that fails too, return 0
                        return GetPeersCountResponse(success=True, peers_count=0)
                else:
    ...
    async def get_certificate(self) -> GetCertificateResponse:
        ...
        if isinstance(result, dict):
            ...
            if result.get("success") or "data" in result:
                ...
                if "404" in str(result.get("error", "")):
                    ...
                    return GetCertificateResponse(
                        success=True, certificate="", expires_at=None
                    )
                else:
                    # Other error, raise it
        ...
        if "404" in str(result):
            # Certificate not found is acceptable - return empty certificate
            return GetCertificateResponse(success=True, certificate="", expires_at=None)
