...
class BlobManager:
    ...
    async def upload(self, data: bytes, metadata: bytes = b"") -> UploadBlobResponse:
        ...
        """
        request = UploadBlobRequest(data=data, metadata=metadata)
        ...
        if isinstance(result, dict):
            ...
            if result.get("success") or "data" in result:
                ...
                return ListBlobsResponse(
                    success=True, blobs=blobs, total_count=len(blobs)
                )
    ...
    async def get_info(self, blob_id: str) -> GetBlobInfoResponse:
        ...
        result = await self.client._make_request(
            "GET", f"/admin-api/blobs/{blob_id}/info"
        )
        if isinstance(result, dict) and (result.get("success") or "data" in result):
            ...
            return GetBlobInfoResponse(success=True, blob=blob_info)
        else:
    ...
    async def delete(self, blob_id: str) -> DeleteBlobResponse:
        ...
        """
        result = await self.client._make_request(
            "DELETE", f"/admin-api/blobs/{blob_id}"
        )
        if isinstance(result, dict) and (result.get("success") or "data" in result):
            return DeleteBlobResponse(success=True, blob_id=blob_id)
        else:
