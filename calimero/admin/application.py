...
class ApplicationManager:
    ...
    async def install_dev(
        ...
        """
        request = InstallDevApplicationRequest(path=path, metadata=metadata)
        return await self._install_dev_typed(request)

    async def _install_dev_typed(
        ...
        result = await self.client._make_request(
            "POST", "/admin-api/install-dev-application", payload
        )
    ...
    async def install_from_url(
        ...
        """
        request = InstallApplicationRequest(url=url, hash=hash, metadata=metadata)
        return await self._install_typed(request)

    async def _install_typed(
        self, request: InstallApplicationRequest
    ) -> InstallApplicationResponse:
        """
        ...
        result = await self.client._make_request(
            "POST", "/admin-api/install-application", payload
        )
        if isinstance(result, dict) and (result.get("success") or "data" in result):
            # Add success field if it doesn't exist, so the workflow engine can access it
            if "success" not in result:
    ...
    async def list_all(self) -> ListApplicationsResponse:
        ...
        if isinstance(result, dict):
            ...
            if result.get("success") or "data" in result:
                ...
                applications_data = (
                    data.get("apps", data) if isinstance(data, dict) else data
                )
                ...
                applications = [
                    ApplicationInfo(
                        ...
                        status=app.get(
                            "status", "installed"
                        ),  # Default to 'installed' if not provided
                        installed_at=app.get("installedAt"),
                        metadata=(
                            bytes(app.get("metadata", []))
                            if app.get("metadata")
                            else None
                        ),
                    )
    ...
    async def get(self, application_id: str) -> GetApplicationResponse:
        ...
        """
        result = await self.client._make_request(
            "GET", f"/admin-api/applications/{application_id}"
        )
        if isinstance(result, dict) and (result.get("success") or "data" in result):
            ...
            application = ApplicationInfo(
                ...
                metadata=(
                    bytes(app_data.get("metadata", []))
                    if app_data.get("metadata")
                    else None
                ),
            )
            return GetApplicationResponse(success=True, application=application)
        else:
    ...
    async def uninstall(self, application_id: str) -> UninstallApplicationResponse:
        ...
        """
        result = await self.client._make_request(
            "DELETE", f"/admin-api/applications/{application_id}"
        )
        if isinstance(result, dict) and (result.get("success") or "data" in result):
