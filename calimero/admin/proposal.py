...
class ProposalManager:
    ...
    async def list_all(
        self, context_id: str, offset: int = 0, limit: int = 100
    ) -> GetProposalsResponse:
        """
        ...
        result = await self.client._make_request(
            "GET", f"/admin-api/contexts/{context_id}/proposals", params
        )
    ...
    async def get(self, context_id: str, proposal_id: str) -> GetProposalResponse:
        ...
        """
        result = await self.client._make_request(
            "GET", f"/admin-api/contexts/{context_id}/proposals/{proposal_id}"
        )
        if isinstance(result, dict) and (result.get("success") or "data" in result):
            # Add success field if it doesn't exist, so the workflow engine can access it
            if "success" not in result:
    ...
    async def get_active_count(
        ...
        """
        result = await self.client._make_request(
            "GET", f"/admin-api/contexts/{context_id}/proposals/active/count"
        )
        if isinstance(result, dict) and (result.get("success") or "data" in result):
            # Add success field if it doesn't exist, so the workflow engine can access it
            if "success" not in result:
    ...
    async def get_approvals_count(
        self, context_id: str, proposal_id: str
    ) -> GetProposalApprovalsCountResponse:
        """
        ...
        result = await self.client._make_request(
            "GET",
            f"/admin-api/contexts/{context_id}/proposals/{proposal_id}/approvals/count",
        )
        if isinstance(result, dict) and (result.get("success") or "data" in result):
            # Add success field if it doesn't exist, so the workflow engine can access it
            if "success" not in result:
    ...
    async def get_approvers(
        ...
        """
        result = await self.client._make_request(
            "GET", f"/admin-api/contexts/{context_id}/proposals/{proposal_id}/approvers"
        )
        if isinstance(result, dict) and (result.get("success") or "data" in result):
            # Add success field if it doesn't exist, so the workflow engine can access it
            if "success" not in result:
