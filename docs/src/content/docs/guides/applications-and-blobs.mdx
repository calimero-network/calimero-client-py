...
Install from a URL. The optional `hash` (a hex-encoded 32-byte digest) pins the
...
:::note
`hash` must be a hex string that decodes to exactly 32 bytes, or the call raises
an error. Leave it `None` to skip integrity pinning.
:::
