...
# This script builds both the Python client and the Rust bindings
...
if [ ! -f "pyproject.toml" ]; then
    echo "❌ Error: pyproject.toml not found. Please run this script from the calimero-client-py root directory."
    exit 1
fi
...
rm -rf target/ dist/ build/ *.egg-info/ bindings/target/ bindings/dist/
...
# Build the Python package with bindings
