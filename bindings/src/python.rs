...
use calimero_client::{CliAuthenticator, ClientError, JwtToken};

/// Python module for Calimero client bindings
#[pymodule]
fn calimero_client_py_bindings(_py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    // Register classes
...
impl PyClient {
    ...
    fn invite_to_context(
        ...
        Python::with_gil(|py| {
            let result = self.runtime.block_on(async move {
                let request = admin::InviteToContextRequest::new(
        ...
        Python::with_gil(|py| {
            let result = self.runtime.block_on(async move {
                ...
                let request = admin::JoinContextRequest::new(
                    ContextInvitationPayload::try_from(
                        invitation_payload,
                    )
    ...
    fn update_context_application(
        ...
        Python::with_gil(|py| {
            let result = self.runtime.block_on(async move {
                let request =
                    admin::UpdateContextApplicationRequest::new(
                        application_id,
                        executor_public_key,
                    );
    ...
    fn create_alias_generic(
        ...
        Python::with_gil(|py| {
            let result = self.runtime.block_on(async move {
                ...
                let alias_obj =
                    Alias::<ContextId>::from_str(&alias_str)
                        .map_err(|e| eyre::eyre!("Invalid alias: {}", e))?;
