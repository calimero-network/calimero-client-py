...
class TestIdentityWorkflows:
    ...
    async def test_identity_generation_workflow(self, workflow_environment):
        ...
        if isinstance(identity_result, dict):
            if 'data' in identity_result and 'publicKey' in identity_result['data']:
                ...
                raise ValueError(f"Unexpected identity result format: {identity_result}")
        else:
        ...
        assert public_key.isalnum() or all(c in '123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz' for c in public_key)
    ...
    async def test_identity_context_integration(self, workflow_environment):
        ...
        elif isinstance(identities, dict) and 'data' in identities and 'identities' in identities['data']:
    ...
    async def test_identity_workflow_verification(self, workflow_environment):
        ...
        admin_url = env.endpoints["calimero-node-2"]  # Use node 2 where identity was created
