...
class TestApplicationWorkflows:
    ...
    async def test_application_installation_workflow(self, workflow_environment):
        ...
        prod_result = await client.applications.install_from_url(app_url, metadata=app_metadata)
        assert prod_result.get('success') or 'applicationId' in prod_result
        prod_app_id = prod_result.get('applicationId') or prod_result.get('data', {}).get('applicationId')
        ...
        print(f"✅ Applications listed: {len(apps.applications) if hasattr(apps, 'applications') else 'N/A'} total")
    ...
    async def test_application_metadata_handling(self, workflow_environment):
        ...
        for i, metadata in enumerate(metadata_cases):
            result = await client.applications.install_from_url(app_url, metadata=metadata)
    ...
    async def test_application_workflow_integration(self, workflow_environment):
        ...
        apps = await client.applications.list_all()
        app_ids = [app.id for app in apps.applications] if hasattr(apps, 'applications') else []
