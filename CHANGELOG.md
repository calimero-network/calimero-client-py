...
- build(deps)!: move the lock from `cdfe5a82` to `2d05a70e` — calimero-network/core#3431, one root key is one account everywhere. 0.6.26 shipped against the commit immediately before it
- fix(account)!: `pair_device_init` no longer takes an account nonce. An account is the content address of its root key, so the root key is the whole of what has to travel between two devices, and the request type has no field for a nonce any more. Accepted and discarded for this release so a caller still passing it positionally gets a no-op rather than a `TypeError`; it goes in the next one
- `create_namespace` still builds its body by hand. Its condition is "remove once no supported release predates the removal", and the latest release (`0.11.0-rc.20`, 6 Aug) does predate it — calimero-network/core#3396 cuts rc.21, which carries it

## 0.6.26

- build(deps)!: bump the core dependency from `fc4babde2` to `cdfe5a82`. The lock was the reason four bindings could not use the typed path: at the old revision `RemoveGroupMembersApiRequest::members` was still `Vec<PublicKey>` and `AccountId` did not exist in `calimero-primitives` at all, so the structs genuinely could not express the ids the node uses. **This also picks up the removal of `selfIdentity` from `list_group_members`** — a caller looking for itself in a member list now asks `get_namespace_identity` and matches its `account` against `members[].identity`, one id space on both sides
